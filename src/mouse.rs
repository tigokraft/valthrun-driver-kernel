...
use winapi::{shared::ntdef::{UNICODE_STRING, PVOID}, km::wdm::DRIVER_OBJECT};

use crate::{kdef::{MouseClassServiceCallbackFn, IoDriverObjectType, MOUSE_INPUT_DATA, MOUSE_BUTTON_LEFT_UP, MOUSE_BUTTON_LEFT_DOWN, MOUSE_BUTTON_RIGHT_UP, MOUSE_BUTTON_RIGHT_DOWN, MOUSE_BUTTON_MIDDLE_UP, MOUSE_BUTTON_MIDDLE_DOWN, MOUSE_BUTTON_4_UP, MOUSE_BUTTON_4_DOWN, MOUSE_BUTTON_5_UP, MOUSE_BUTTON_5_DOWN, MOUSE_BUTTON_WHEEL, MOUSE_BUTTON_HWHEEL}, kapi::{KModule, Object, UnicodeStringEx}, offsets::NtOffsets};
...
fn find_mouse_service_callback() -> anyhow::Result<MouseClassServiceCallbackFn> {
    ...
    let pattern = ByteSequencePattern::parse(obfstr!("48 8D 05 ? ? ? ? 48 89 44"))
        .with_context(|| obfstr!("Failed to compile MouseClassServiceCallback pattern").to_string())?;

    NtOffsets::locate_function(
        &module_kdbclass, obfstr!("MouseClassServiceCallback"), 
        &pattern, 0x03, 0x07
    )
}
...
pub fn create_mouse_input() -> anyhow::Result<MouseInput> {
    ...
    let mouse_device = match mouse_device {
        ...
        None => anyhow::bail!("no mouse device detected")
    };
