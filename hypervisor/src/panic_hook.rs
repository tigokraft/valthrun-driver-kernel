...
static PANIC_INITIATED: AtomicBool = AtomicBool::new(false);

pub fn dbg_break() {
    unsafe {
        (DEBUG_IMPORTS.unwrap().DbgBreakPoint)();
    }
}
...
fn panic(info: &PanicInfo) -> ! {
     ...
     */
    PANIC_INITIATED.store(true, Ordering::Relaxed);
...
pub unsafe extern "C" fn __CxxFrameHandler3() {
    if PANIC_INITIATED.load(Ordering::Relaxed) {
        return;
    }
    ...
    unsafe {
        (imports.DbgPrintEx)(
            0,
            DPFLTR_LEVEL::ERROR as u32,
            obfstr!("[VTHV] __CxxFrameHandler3 has been called. This should no occur.\n\0")
                .as_ptr(),
        );
        (imports.DbgBreakPoint)();
